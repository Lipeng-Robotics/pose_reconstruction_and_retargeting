...
from h2tc_fit_dataset import H2TCFitDataset
...
from video_process import video_to_images, run_openpose, run_deeplab_v3
...
def main(args, config_file):
    ...
    if not os.path.exists(video_preprocess_path) and (not use_custom_keypts and not os.path.exists(op_out_path)):
        mkdir(video_preprocess_path)
        # video -> images (at 30 Hz) - save to new output directory
        if '.mp4' in args.data_path: 
    ...
    if use_custom_keypts:
        op_out_path = os.path.join(args.data_path, args.op_keypts)
        assert os.path.exists(op_out_path), 'Could not find custom keypoints path %s!' % (op_out_path)
        print('Using pre-detected OpenPose keypoints from %s...' % (op_out_path))

    # Create dataset by splitting the video up into overlapping clips
    vid_name = '.'.join(args.data_path.split('/')[-1].split('.')[:-1])
    dataset = H2TCFitDataset(op_out_path,
                                cam_mat=None,
    ...
    for i, data in enumerate(data_loader):
        ...
        try:
            ...
            if args.vis:
                vis_results_lx(cur_res_out_paths, optim_result, per_stage_results, gt_data, observed_data, args.data_type,
                                        device,
                                        optim_floor=optimizer.optim_floor,
                                        obs_img_paths=obs_img_paths,
                                        obs_mask_paths=obs_mask_paths,
