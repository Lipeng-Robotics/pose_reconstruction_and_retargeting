...
from human_tools.body_model import SMPL_JOINTS, SMPL_PARENTS
...
class FittingLoss(nn.Module):
    ...
    def __init__(self, loss_weights,
        ...
        if use_chamfer:
            from motion_modeling.util_tool.chamfer_distance import ChamferDistance
    ...
    def optitrack_fit(self, observed_data, pred_data, flag_rhand, flag_lhand):
        ...
        # loss = loss_head + loss_rhand + loss_lhand
        if self.args.is_sub1 == "sub1":
            loss = loss_rhand + loss_lhand
        else:
            gt_head = observed_data['sub2_head'][0,:,:3] # [seq_num, 3, 1]
            smpl_head = pred_data['points3d'][0, :, 411, :] # head id is 411 [seq_num, 3]
            loss = torch.sum((gt_head[:,0] - smpl_head[:,0])**2) + torch.sum((gt_head[:,2] - smpl_head[:,2])**2) 
