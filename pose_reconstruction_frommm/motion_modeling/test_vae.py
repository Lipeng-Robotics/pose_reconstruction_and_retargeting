...
from util_tool.config import TestConfig
from util_tool.logging import Logger, class_name_to_file_name, mkdir, cp_files
from util_tool.torch import get_device, save_state, load_state
from util_tool.stats import StatTracker
from util_tool.transforms import rotation_matrix_to_angle_axis
from human_tools.body_model import SMPL_JOINTS
from h2tc_dataset import NUM_KEYPT_VERTS, CONTACT_INDS
from humor_loss import CONTACT_THRESH
...
def test(args_obj, config_file):
    ...
    model_class = importlib.import_module('humor_model')
    Model = getattr(model_class, 'HumorModel')
    ...
    loss_class = importlib.import_module('humor_loss')
    Loss = getattr(loss_class, 'HumorLoss')
    ...
    ckpt_root = "./out/humor_train"
    ckpt_folder = os.listdir(ckpt_root)[-1]
    ckpt_file = os.path.join(ckpt_root, ckpt_folder, "checkpoints/best_train_model.pth")
    args.ckpt = ckpt_file
    start_epoch, min_val_loss, min_train_loss = load_state(ckpt_file, model, optimizer=None, map_location=device, ignore_keys=model.ignore_keys)
    ...
    Dataset = getattr(importlib.import_module('h2tc_dataset'), 'H2tcDataset')
    test_dataset = Dataset(split='test', **args_obj.dataset_dict)
    # create loaders
...
def eval_sampling(model, test_dataset, test_loader, device, 
    ...
    from human_tools.body_model import BodyModel, male_bm_path, female_bm_path
...
def viz_eval_samp(global_gt_dict, x_pred_dict, meta, male_bm, female_bm, out_path_list,
    ...
    for b in range(B):
        ...
        # pred
        trans_zero = torch.zeros_like(pred_world_trans[b])
        ...
        from util_tool.viz import viz_smpl_seq, create_video
